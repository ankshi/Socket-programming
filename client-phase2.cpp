...
try
    {
        ...
        else
        {
            ...
            vector<string> client_data=split_string(line);
           
            //collecting my data
            CLIENT_ID=stoi(client_data[0]);
            
            CLIENT_PORT=stoi(client_data[1]);
            
            CLIENT_UID=stoi(client_data[2]);
...
void print_needed_file_info(vector<Packet> FILES_TO_SEARCH,string DIRECTORY_PATH )
{
    char info_buffer[2000];
    char* file_buffer;//cout<<"here\n";
   for(auto itr=FILES_TO_SEARCH.begin();itr!=FILES_TO_SEARCH.end();itr++)
   {
       string filename=itr->filename;
       int owner_uid=itr->uid;
       int depth=itr->depth;
       if(owner_uid!=0)
       {
       
       string filepath=DIRECTORY_PATH+"/Downloaded/"+filename;//cout<<filepath<<"\n";
       int file_descript = open(filepath.c_str(), O_RDONLY);
       if(file_descript < 0) {cout<<"yahan haga";exit(-1);}
       ...
       cout<<"Found "<<filename<<" at "<<owner_uid<<" with MD5 ";
       print_md5_sum(result);
       cout<<" at depth "<<depth<<endl;
       filepath.clear();
       //sprintf(info_buffer,"Found %s at %d with MD5 0 at depth %d \n",filename.c_str(),owner_uid,depth);
       
       }
       else{
          sprintf(info_buffer,"Found %s at %d with MD5 0 at depth %d \n",filename.c_str(),owner_uid,depth); 
          cout<<info_buffer; memset(info_buffer,0,2000);
       }
...
void print_needed_file_info_no_hash(vector<Packet> FILES_TO_SEARCH )
{
    char info_buffer[2000];char* file_buffer;
   for(auto itr=FILES_TO_SEARCH.begin();itr!=FILES_TO_SEARCH.end();itr++)
   {
       string filename=itr->filename;
       int owner_uid=itr->uid;
...
void display(queue<Packet> dis)
{
   for(int i=0;i>dis.size();i++)
   {
    Packet temp=dis.front();
    cout<<temp.filename<<" ";
    dis.pop();
   }
  cout<<"\n";
...
//just for phase2-3
void print_needed_file_info_only(vector<Packet> FILES_TO_SEARCH )
{
    char info_buffer[2000];
   for(auto itr=FILES_TO_SEARCH.begin();itr!=FILES_TO_SEARCH.end();itr++)
   {
